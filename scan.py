...
INDUSTRY_QUERY_MAX_ATTEMPTS = 2
INDUSTRY_QUERY_RETRY_WAIT = 0.8
MIN_PRICE = 3
MAX_PRICE = 100
...
def match_stock_industry(
    ...
    if cached:
        cached_key = normalize_industry_text(
            cached
        )

        if cached_key in normalized_industries:
            industry = (
                normalized_industries[
                    cached_key
                ]
        ...
        remapped = industry_keyword_match(
            cached,
            industries
        )
...
def enrich_industry_info(records):
    ...
    try:
        ...
        for item in records:
            ...
            item["industry"] = None
            item["industry_match_field"] = None
            item["industry_raw_class"] = None
    ...
    print(
        "行业匹配统计："
        f"缓存命中={cache_hits} | "
        f"联网新增={network_matches} | "
        f"查询失败={query_failures} | "
